...
public:
    explicit __WFDynamicTask(std::function<SubTask *(WFDynamicTask *)> &&create) : create(std::move(create)) {}
};
...
void WFComplexClientTask<REQ, RESP, CTX>::router_callback(void *t) {
    ...
    if (this->state == WFT_STATE_SUCCESS) route_result_ = std::move(*task->get_result());
    else if (this->state == WFT_STATE_UNDEFINED) {
        /* should not happend */
        this->state = WFT_STATE_SYS_ERROR;
        this->error = ENOSYS;
    } else this->error = task->get_error();
}

template <class REQ, class RESP, typename CTX>
void WFComplexClientTask<REQ, RESP, CTX>::dispatch() {
    switch (this->state) {
    case WFT_STATE_UNDEFINED: if (this->check_request()) {
...
SubTask *WFComplexClientTask<REQ, RESP, CTX>::done() {
    ...
    } else { this->switch_callback(nullptr); }
