...
static int __encode_auth(const char *p, std::string &auth) {
    ...
    const size_t base64_len = (len + 2) / 3 * 4;
...
public:
    ComplexHttpTask(int redirect_max, int retry_max,
...
bool ComplexHttpTask::need_redirect(const ParsedURI &uri, ParsedURI &new_uri) {
    ...
    case 301:
    case 302:
    case 303: if (redirect_url(client_resp, uri, new_uri)) {
    ...
    case 307:
    case 308: if (redirect_url(client_resp, uri, new_uri)) {
            return true;
        } else {
            break;
        }
...
private:
    ...
    [[nodiscard]] SSLHandshaker *get_ssl_handshaker() const {
        return &static_cast<SSLConnection *>(this->get_connection())->handshaker; // 为什么不用dynamic_cast???
    }

    SSLWrapper *get_ssl_wrapper(ProtocolMessage *msg) const {
        auto *conn = static_cast<SSLConnection *>(this->get_connection()); // 为什么不用dynamic_cast???
...
CommMessageOut *ComplexHttpProxyTask::message_out() {
    ...
    auto *msg = static_cast<ProtocolMessage *>(this->ComplexHttpTask::message_out()); // 为什么不用 dynamic_cast???
...
CommMessageIn *ComplexHttpProxyTask::message_in() {
    ...
    auto *msg = static_cast<ProtocolMessage *>(this->ComplexHttpTask::message_in()); // 为什么不用 dynamic_cast???
    return is_ssl_ ? get_ssl_wrapper(msg) : msg;
}
...
int ComplexHttpProxyTask::keep_alive_timeout() {
    ...
    if (seqid == 0) {
        ...
        *resp = std::move(*(HttpResponse *)this->get_message_in());
        ...
        switch (status_code) {
        case 200: break;
        case 407: this->disable_retry();
        default: state_ = WFT_STATE_TASK_ERROR;
...
bool ComplexHttpProxyTask::finish_once() {
    ...
    if (this->state != WFT_STATE_SUCCESS) this->check_response();
    ...
    if (this->state == WFT_STATE_SUCCESS) {
        ...
        } else if (this->state != WFT_STATE_SUCCESS) this->disable_retry();
    }
