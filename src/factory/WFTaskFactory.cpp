...
protected:
    void dispatch() override {
        if (this->scheduler->sleep(this) >= 0) this->cancel();
        else this->handle(WFT_STATE_SYS_ERROR, errno);
    }
...
public:
    ...
    virtual ~__WFNamedTimerTask() {
        if (node_.task) {
            ...
            if (node_.task) erased = timers_->del(&node_, &__timer_map.root_);

            __timer_map.mutex_.unlock();
            if (erased) delete timers_;
        }
    }
...
void __WFNamedTimerTask::dispatch() {
    ...
    if (ret >= 0 && flag_.exchange(true)) this->cancel();

    mutex_.unlock();
    if (ret < 0) this->handle(WFT_STATE_SYS_ERROR, errno);
}
...
int __NamedTimerMap::cancel(const std::string &name, size_t max) {
    ...
    if (timers) {
        while (true) {
            ...
            list_del(&node->list);
            if (node->task->flag_.exchange(true)) node->task->cancel();
...
int __NamedCounterMap::count_n(const std::string &name, unsigned int n) {
    ...
    if (counters) erased = count_n_locked(counters, n, &task_list);
...
void __NamedCounterMap::count(CounterList *counters,
    ...
    if (task) task->WFCounterTask::count();
}
...
public:
    ...
    ~__WFNamedMailboxTask() override {
        if (!this->flag) __mailbox_map.remove(mailboxes_, &node_);
    }
...
int __NamedMailboxMap::send(const std::string &name, void *const msg[],
    ...
    if (mailboxes) erased = send_max_locked(mailboxes, max, &task_list);
...
public:
    ...
    ~__WFNamedConditional() override {
        if (!this->flag) __conditional_map.remove(conds_, &node_);
    }
...
int __NamedConditionalMap::signal(const std::string &name, void *const msg[],
    ...
    if (conds) erased = signal_max_locked(conds, max, &cond_list);

    mutex_.unlock();
    if (erased) delete conds;
...
public:
    ...
    ~__WFNamedGuard() override {
        if (!this->flag) __guard_map.unref(guards_);
    }
...
void __WFNamedGuard::dispatch() {
    ...
    if (guards_->acquired) {
        ...
        this->WFConditional::signal(nullptr);
    }
...
struct __guard_node *__NamedGuardMap::release(const std::string &name) {
    struct __guard_node *node = nullptr;
    GuardList *guards;
