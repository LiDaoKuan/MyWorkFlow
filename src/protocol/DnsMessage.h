...
#ifndef MYWORKFLOW_DNSMESSAGE_H
#define MYWORKFLOW_DNSMESSAGE_H
...
namespace protocol {
    ...
    public:
        ...
        [[nodiscard]] int get_aa() const {
            return parser->header.aa;
        }

        [[nodiscard]] int get_tc() const {
            return parser->header.tc;
        }

        [[nodiscard]] int get_rd() const {
            return parser->header.rd;
        }

        [[nodiscard]] int get_ra() const {
            return parser->header.ra;
        }

        [[nodiscard]] int get_rcode() const {
            return parser->header.rcode;
        }

        [[nodiscard]] int get_qdcount() const {
            return parser->header.qdcount;
        }

        [[nodiscard]] int get_ancount() const {
            return parser->header.ancount;
        }

        [[nodiscard]] int get_nscount() const {
            return parser->header.nscount;
        }

        [[nodiscard]] int get_arcount() const {
        ...
        [[nodiscard]] int get_question_type() const {
            return parser->question.qtype;
        }

        [[nodiscard]] int get_question_class() const {
            return parser->question.qclass;
        }

        [[nodiscard]] std::string get_question_name() const {
        ...
        int add_a_record(int section, const char *name, uint16_t rclass, uint32_t ttl, const void *data);

        int add_aaaa_record(int section, const char *name, uint16_t rclass, uint32_t ttl, const void *data);

        int add_ns_record(int section, const char *name, uint16_t rclass, uint32_t ttl, const char *data);

        int add_cname_record(int section, const char *name, uint16_t rclass, uint32_t ttl, const char *data);

        int add_ptr_record(int section, const char *name, uint16_t rclass, uint32_t ttl, const char *data);

        int add_soa_record(int section, const char *name, uint16_t rclass, uint32_t ttl,
                           const char *mname, const char *rname, uint32_t serial, int32_t refresh,
                           int32_t retry, int32_t expire, uint32_t minimum);

        int add_srv_record(int section, const char *name, uint16_t rclass, uint32_t ttl,
                           uint16_t priority, uint16_t weight, uint16_t port, const char *target);

        int add_mx_record(int section, const char *name, uint16_t rclass, uint32_t ttl, int16_t preference, const char *exchange);
        ...
        // Inner use only
        [[nodiscard]] bool is_single_packet() const {
            return parser->single_packet;
        }

        void set_single_packet(const bool single) {
            parser->single_packet = single;
        }

    public:
        DnsMessage() : parser(new dns_parser_t) {
        ...
        DnsMessage(DnsMessage &&msg) noexcept;
        DnsMessage &operator =(DnsMessage &&msg) noexcept;
    ...
    private:
        int encode_reply();
        int encode_truncation_reply(); // 暂未实现
        list_head *get_section(int section) const;
    ...
    public:
        ...
        DnsResponse &operator=(DnsResponse &&) = default;

        [[nodiscard]] const dns_parser_t *get_parser() const {
            return this->parser;
        }
