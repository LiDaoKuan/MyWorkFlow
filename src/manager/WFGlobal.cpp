...
private:
    __SSLManager() {
        ...
        if (ssl_client_ctx_ == nullptr) { abort(); }
    }
    ...
    if (!fio_flag_) {
        ...
        if (!fio_flag_) {
            ...
            while (fio_service_->init(maxevents) < 0) {
                if ((errno != EAGAIN && errno != EINVAL) || maxevents <= 16) abort();

                while (n >= maxevents) n /= 2;

                maxevents = n;
            }

            if (fio_service_->bind() < 0) abort();
...
private:
    __ExecManager() :
        ...
        if (compute_threads < 0) compute_threads = sysconf(_SC_NPROCESSORS_ONLN);

        if (compute_executor_.init(compute_threads) < 0) abort();
    }
...
inline ExecQueue *__ExecManager::get_exec_queue(const std::string &queue_name) {
    ...
    if (iter != queue_map_.cend()) queue = iter->second;
    ...
    if (iter == queue_map_.cend()) {
        queue = new ExecQueue();
        if (queue->init() >= 0) queue_map_.emplace(queue_name, queue);
        else {
            delete queue;
            queue = nullptr;
        }
    } else queue = iter->second;
...
static std::string __dns_server_url(const std::string &url,
    ...
    if (strncasecmp(url.c_str(), "dns://", 6) == 0 ||
        ...
        host = url;
    } else if (inet_pton(AF_INET6, url.c_str(), &buf) > 0) host = "dns://[" + url + "]";
    else host = "dns://" + url;
...
static void __split_merge_str(const char *p, bool is_nameserver,
    ...
    while (true) {
        ...
        if (is_nameserver) str = __dns_server_url(str, hints);
...
static int __parse_resolv_conf(const char *path,
    ...
    while ((ret = getline(&line, &bufsize, fp)) > 0) {
        if (strncmp(line, "nameserver", 10) == 0) __split_merge_str(line + 10, true, &hints, url);
        else if (strncmp(line, "search", 6) == 0) __split_merge_str(line + 6, false, &hints, search_list);
        else if (strncmp(line, "options", 7) == 0) __set_options(line + 7, ndots, attempts, rotate);
    }
...
void WFGlobal::sync_operation_end(int cookie) {
    if (cookie) __WFGlobal::get_instance()->sync_operation_end();
}
