...
static void __callback_internal(WFDnsTask *task, const DnsParams &params, struct DnsStatus &s) {
    ...
    const int rcode = resp->get_rcode();

    const bool try_next_server = state != WFT_STATE_SUCCESS || rcode == DNS_RCODE_SERVER_FAILURE ||
                                 rcode == DNS_RCODE_NOT_IMPLEMENTED || rcode == DNS_RCODE_REFUSED;
    const bool try_next_name = rcode == DNS_RCODE_FORMAT_ERROR || rcode == DNS_RCODE_NAME_ERROR ||
                               resp->get_ancount() == 0;
...
int WFDnsClient::init(const std::string &url, const std::string &search_list,
    ...
    DnsParams::dns_params *q = static_cast<DnsParams *>(this->params)->get_params();
...
WFDnsTask *WFDnsClient::create_dns_task(const std::string &name, dns_callback_t callback) {
    const DnsParams::dns_params *p = static_cast<DnsParams *>(this->params)->get_params();
    ...
    ComplexTask *ctask = static_cast<ComplexTask *>(task); // 为什么不用dynamic_cast???
